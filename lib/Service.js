 ...
 */
class Service {
  constructor ({ serviceName, servicePath, host, domain, token, rootCerts, secure }) {
  ...
  send ({
    ...
    return new Promise((resolve, reject) => {
      // 发送请求
      client[method](message, metaData, function (err, res) {
        if (err) {
          reject(err)
          return
        }
  ...
  call ({
    ...
    const call = client[method](message, metaData)

    return new StreamCall(call, client)
  }
