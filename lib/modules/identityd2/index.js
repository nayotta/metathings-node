...
class Identityd2Service {
  constructor (host, token, rootCerts) {
    this._host = host
    this._token = token || ''
    this._metaData = new grpc.Metadata()
    if (rootCerts !== undefined && rootCerts.byteLength === undefined) {
    ...
    if (rootCerts) {
      ...
      const issuer = x509.getIssuer(rootCerts.toString())
      const { commonName } = issuer
      if (commonName) options['grpc.ssl_target_name_override'] = commonName
    }
    this._client = new services.IdentitydServiceClient(host, channelCredentials || grpc.credentials.createInsecure(), rootCerts)
  }
